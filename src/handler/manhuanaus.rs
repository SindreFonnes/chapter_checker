...
pub fn check(text: String, url: String) -> Result<f32, CheckError> {
    let parse_error_message = format!("Error parsing manhuaus: {}", url);

    let text = text.split("LATEST MANGA RELEASES").collect::<Vec<&str>>();
    ...
    let text = text[2].split("href=\"https://").collect::<Vec<&str>>();
    ...
    let chapter = chapter
        .parse::<f32>()
        .map_err(|err| CheckError::Parse(format!("Couldn't parse float manhuaus {err} {}", url)))?;
