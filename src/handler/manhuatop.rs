...
pub fn check(text: String, url: String) -> Result<f32, CheckError> {
    println!("/n/n/n/n/n HERE /n/n/n/n/n");
    let parse_error_message = format!("Error parsing manhuatop: {}", url);
    ...
    let chapter = chapter.parse::<f32>().map_err(|err| {
        CheckError::Parse(format!("Couldn't parse float manhuatop {err} {}", url))
    })?;
